...
=============================================================================*/
// Stage 3 of the synth: a chorus after the voices. Stage 2's cases carry
...
namespace
{
   ...
   constexpr clap_id chorus_rate_param = 15;
   constexpr clap_id chorus_depth_param = 16;
...
////////////////////////////////////////////////////////////////////////////
// The chorus: a swept delay mixed in after the voices.
////////////////////////////////////////////////////////////////////////////
namespace
{
   // A held note, filter open and flat, chorus set as given.
   std::vector<float> held_with_chorus(
      instance& synth, double rate, double depth_s, double mix)
   {
      hold_flat(synth);
      param_events r{chorus_rate_param, rate};
      synth.run(&r._in);
      param_events d{chorus_depth_param, depth_s};
      synth.run(&d._in);
      param_events m{chorus_mix_param, mix};
      synth.run(&m._in);
      note_events on{69, 0, true};
      synth.run(&on._in);
      collect(synth, 5);
      return collect(synth, 40);
   }
}

TEST_CASE("With the chorus dry, a held note is steady")
{
   instance synth;
   auto const out = held_with_chorus(synth, 1.0, 0.005, 0.0);
   CHECK(period_spread(out) < 1.02f);
}

TEST_CASE("With the chorus wet and swept, a held note wavers")
{
   // The delayed copy alone, sweeping: what comes out is the note with
   // vibrato, which is what a chorus is made of.
   instance synth;
   auto const out = held_with_chorus(synth, 3.0, 0.005, 100.0);
   CHECK(period_spread(out) > 1.02f);
}

TEST_CASE("With the chorus wet but not swept, a held note is steady")
{
   // Depth is what makes it waver; a fixed delay changes nothing the
   // ear can time.
   instance synth;
   auto const out = held_with_chorus(synth, 3.0, 0.0, 100.0);
   CHECK(period_spread(out) < 1.02f);
}

TEST_CASE("Half mix is neither the dry note nor the wet one")
{
   instance dry, wet, half;
   auto const a = held_with_chorus(dry, 3.0, 0.005, 0.0);
   auto const b = held_with_chorus(wet, 3.0, 0.005, 100.0);
   auto const c = held_with_chorus(half, 3.0, 0.005, 50.0);

   auto differs = [](std::vector<float> const& x, std::vector<float> const& y)
   {
      float d = 0;
      for (std::size_t i = 0; i != x.size(); ++i)
         d += std::abs(x[i] - y[i]);
      return d / float(x.size());
   };
